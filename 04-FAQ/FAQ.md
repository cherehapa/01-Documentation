...
Это стоимость полиса для одного туриста. Если передан запрос на расчет страховки для нескольких туристов, то поле quote->data->price будет содержать суммарную стоимость расчета, а в quote->data->infoPerTourist->perTourist - стоимость страховки для конкретного туриста. Она может варьироваться в зависимости от возраста туриста - для пожилых туристов она дороже.
Например:
...
У одного туриста, 28 лет - 2.86 у.е., у другого, пожилого, 11.44 у.е.
...
Так же можно использовать и при проведении повторных расчетов по указанным ранее условиям. Например:
...
Да, полное описание будет, но подготовить сможем только после запуска. Пока могу дать только текстовое описание различных объектов, если нужно.
...
Список страховых компаний для организации интерфейса. К сожалению, точно не знаю, как выглядит ваш интерфейс для выписки страховых полисов, но в своем интерфейсе мы используем список страховых компаний для того, чтобы показать не просто названия продуктов, а еще и названия/логотипы страховых компаний, которые продают каждый из продуктов.
...
Аннуляцию я пока не описал в документации - мы ее только закончили.
DELETE /policy/<id>.
Опишу метод немного позднее.
